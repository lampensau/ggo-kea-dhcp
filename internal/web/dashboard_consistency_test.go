...
func TestDashboardCardMatchesLeasesPage(t *testing.T) {
	...
	card := leaseRowIPs(s.buildDashboardViewWith(t.Context(), views.PageData{}, leases, netSnapshotData{}, false, nil).RecentLeases)
	page := leaseRowIPs(s.unifiedLeaseRowsWithPins(t.Context(), leases, nil, false, nil, nil, nil, nil))
