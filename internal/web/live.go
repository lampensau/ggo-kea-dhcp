...
func regionOnPage(region, page string) bool {
	if region == "state-badge" || region == "sys-health" || region == "backend-alert" || region == "kea-toast" || region == "update-badge" || region == "update-dialogs" || region == "standdown-toast" {
...
func (s *Server) publishDashboardWithLeases(ctx context.Context, leases []kea.ActiveLease) {
	...
	s.rebuildDNSZoneWith(leases, res, s.zone.nextGen())
