...
func TestResumeInterruptedApplyFallsBackToOnboarding(t *testing.T) {
	...
	_ = r.ReconcileApplianceState(ModeConverge, 0)

	if got, _ := r.sqlite.GetState(db.LifecycleStateKey); got != db.StateOnboarding {
...
func TestActiveZeroScopesRescuesToOnboarding(t *testing.T) {
	seed := func(t *testing.T, withProfile bool) *Reconciler {
		r, _ := newTestReconciler(t)
		if err := r.sqlite.SetState(db.LifecycleStateKey, db.StateActive); err != nil {
			t.Fatalf("seed ACTIVE: %v", err)
		}
		if withProfile {
			if _, err := r.sqlite.Exec("INSERT INTO profiles (name, active) VALUES ('empty', 1)"); err != nil {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			r := seed(t, tc.withProfile)
			r.ArmRescue() // the web layer arms it at boot; the bare test Reconciler does not
			_ = r.ReconcileApplianceState(ModeConverge, 0)
			...
			var n int
			if err := r.sqlite.QueryRow("SELECT COUNT(*) FROM audit_log WHERE action = 'RESCUE_ONBOARDING'").Scan(&n); err != nil || n != 1 {
...
func TestActiveZeroScopesApplyDoesNotRescue(t *testing.T) {
	...
	err := r.ReconcileApplianceState(ModeApply, 0)
	if !errors.Is(err, errNoScopes) {
		t.Errorf("ModeApply with zero scopes = %v, want errNoScopes", err)
	}
	if got, _ := r.sqlite.GetState(db.LifecycleStateKey); got != db.StateActive {
...
func TestZeroScopesRescueOnlyOnFirstConverge(t *testing.T) {
	...
	r.rescueArmed.Store(false)

	err := r.ReconcileApplianceState(ModeConverge, 0)
	if !errors.Is(err, errNoScopes) {
	...
	var n int
	_ = r.sqlite.QueryRow("SELECT COUNT(*) FROM audit_log WHERE action = 'RESCUE_ONBOARDING'").Scan(&n)
	if n != 0 {
...
func TestZeroScopesRescueConsumedByAnyBoot(t *testing.T) {
	r, _ := newTestReconciler(t)
	r.ArmRescue()
	if err := r.sqlite.SetState(db.LifecycleStateKey, db.StateOnboarding); err != nil {
		t.Fatalf("seed state: %v", err)
	}
	_ = r.ReconcileApplianceState(ModeConverge, 0) // the boot reconcile, in ONBOARDING
	...
	if st, _ := r.sqlite.GetState(db.LifecycleStateKey); st != db.StateActive {
